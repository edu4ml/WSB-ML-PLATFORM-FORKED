...
from elearning.reporting.entities.student import StudentInCourse, StudentInCourseProgress, StudentInCourseStepProgress
...
class ReportRepository(Repository):
    def get_for_teacher(self):
        assert self.user and self.user.is_teacher(), f"User needs to be a teacher to generate Teacher Report"
        return Teacher(
            uuid=self.user.uuid,
            courses=[Course(
                uuid=course.uuid,
                students=[StudentInCourse(
                    ...
                    progress=StudentInCourseProgress(
                        ...
                        steps=self.get_course_steps_progress_for_user(course, enrolment.user)
                    )
            ...
            try:
                ...
                steps_progress.append(
                    StudentInCourseStepProgress(
                        ...
                        evaluation_status=CourseStepEvaluationStatus.SKIPPED
                    )
                )
